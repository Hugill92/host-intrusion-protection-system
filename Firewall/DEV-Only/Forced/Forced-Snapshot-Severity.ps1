# Forced-Test
# Category: SnapshotEvents
# Requires: None
# Fatal: false
<#
Forced-Snapshot-Severity.ps1

DETERMINISTIC forced DEV test for snapshot severity classification.
Simulates a firewall rule change and classifies severity deterministically.
...
$TestId = "Forced-Snapshot-Severity"
...
try {
    ...
    $target     = "Inbound Firewall Policy"
    $changeType = "FirewallRuleAdded"
    $rule       = "BLOCK_TCP_23"
    $reason     = "Hardening - block legacy service port"

    $severity = switch ($changeType) {
        "FirewallRuleAdded"    { "Informational" }
        "FirewallRuleModified" { "Warning" }
        "FirewallRuleRemoved"  { "Warning" }
        default                { "Unknown" }
    }

    if ($severity -ne "Informational") { throw "Unexpected severity classification: $severity" }

    Log "[EXPECTED EVENT]"
    Log "--------------------------------------------------"
    Log "Event Type : SnapshotSeverityEvaluation"
    Log ""
    Log ("{0,-12}: {1}" -f "Actor",  $actor)
    Log ("{0,-12}: {1}" -f "Target", $target)
    Log ("{0,-12}: {1}" -f "Change", $changeType)
    Log ("{0,-12}: {1}" -f "Rule",   $rule)
    Log ("{0,-12}: {1}" -f "Reason", $reason)
    Log "--------------------------------------------------"
    Log ("Severity : {0}" -f $severity)
    Log ("Meaning  : {0}" -f "Non-destructive policy addition")
    Log "--------------------------------------------------"
    ...
    [pscustomobject]@{
        ...
        Status="PASS"
        ExpectedEvent=@{
            EventType="SnapshotSeverityEvaluation"
            Actor=$actor
            Target=$target
        ...
        EventResult=@{
            Severity=$severity
            Meaning="Non-destructive policy addition"
        }
        Stamp=$Stamp
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUnBHoiVHj5GLVZqs8BeBSgaEh
# LE2gggK1MIICsTCCAZmgAwIBAgIUA+POe3D7qmANSWS/liNWJ/XK6bEwDQYJKoZI
...
# BAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQU8Vu0ME5oCAeEw75SkM5qCpO0rFQwCwYH
# KoZIzj0CAQUABEcwRQIhAMg8U9m+TIZEh66Xq92VooCgdxQZGsbR4uuQ5TaYO9cO
# AiBHxtwmvU4zdkGqlzvfofuwfTArTg5mYSIQgsESr00Tjg==
