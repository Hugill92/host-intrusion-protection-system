...
function Resolve-FirewallCorePolicySource {
  ...
  $candidates = @(
    ...
    (Join-Path $InstallerRoot "Firewall\Policy\FirewallCorePolicy_v1.wfw"),
...
try {
  $wild = Get-ChildItem -LiteralPath (Join-Path $InstallerRoot "Firewall\Policy") -Filter "FirewallCorePolicy*.wfw" -ErrorAction SilentlyContinue |
    Sort-Object Name -Descending | Select-Object -First 1
  if ($wild -and (Test-Path -LiteralPath $wild.FullName)) { return $wild.FullName }
} catch {}
}
